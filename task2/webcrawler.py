...
class WebCrawler:
    ...
    def __init__(self, config: CrawlConfig):
        ...
        # breadth-first frontier stores (url, depth) pairs beginning with the configured seeds
        self.frontier: Deque[Tuple[str, int]] = deque((seed, 0) for seed in config.seeds)
    ...
    def crawl(self) -> None:
        ...
        with self.output_path.open("w", encoding="utf-8") as out_file:
            ...
            while self.frontier and self.doc_id_counter < self.config.max_pages:
                ...
                self._report_progress(doc_id, normalized_url, depth, page_duration)
    ...
    def _report_progress(self, doc_id: int, url: str, depth: int, page_duration: float) -> None:
        """emit a concise status line showing queue depth and timing information."""
        ...
        print(f"{status} | {timing} | {url}", flush=True)
